...
export default function Home() {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/v1/courses/course/public/mastering-flutter-for-mobile-apps`,
          {
            method: 'GET',
            headers: {
    ...
    const fetchOtherCourses = async () => {
      ...
      const defaultCourseSlug = process.env.DEFAULT_COURSE ?? 'mastering-flutter-for-mobile-apps'
      const apiResponse: OtherCoursesApiResponse = await response.json();
      console.log('API Response:', apiResponse);
      if (apiResponse.data) {
        setOtherCourses(apiResponse.data?.docs?.filter((course) => course.slug !== defaultCourseSlug) ?? []);
      }
    };
    fetchCourseData();
    fetchOtherCourses()
